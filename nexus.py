...
if "Command Center" in page:
    ...
    # ── KPIs: prefer live fundamentals, fallback to CSV ──
    if not fund_df.empty:
        total_rev   = fund_df.revenue_B.sum()
        total_mcap  = fund_df.marketCap_B.sum()
        top_row     = fund_df.loc[fund_df.marketCap_B.idxmax()]
        top_mcap_co = top_row['Company']
        top_mcap    = top_row['marketCap_B']
        nvda_row    = fund_df[fund_df.Company=='NVIDIA']
        nvda_ni     = nvda_row['netIncome_B'].values[0] if not nvda_row.empty else 0
        data_label  = "Live · yfinance"
    else:
        latest      = ann_df[ann_df.Year == ann_df.Year.max()]
    ...
    <div class="kpi-row">
      <div class="kpi">
        ...
        <div class="kpi-sub">8 companies · {data_label}</div>
    ...
    with c1:
        ...
        sf(fig, 350).update_layout(
            title=dict(text="Quarterly Revenue ($B) — Live Earnings", font=dict(size=12,color='#6b80a0')),
    ...
    with c1:
        ...
        sf(fig, 340).update_layout(
            title=dict(text="Normalised Stock Performance (Base = 100)", font=dict(size=12,color='#6b80a0')),
            yaxis_title="Indexed Return"
        )
        st.plotly_chart(fig, use_container_width=True, config={'displayModeBar':False})

    with c2:
        latest_yr = ann_df.Year.max()
        ann_latest = ann_df[ann_df.Year==latest_yr].copy()
        ann_latest['Margin'] = (ann_latest.NetIncome_B / ann_latest.Revenue_B * 100).round(1)
        ann_latest = ann_latest[ann_latest.Company.isin(sel_companies)].sort_values('Margin')
        fig = go.Figure(go.Bar(
            x=ann_latest.Margin, y=ann_latest.Company, orientation='h',
            marker=dict(
                color=ann_latest.Margin,
                colorscale=[[0,'#ff375f'],[0.4,'#ff6d2d'],[1,'#00ff9d']],
                line=dict(width=0)
            ),
            text=[f"{v:.1f}%" for v in ann_latest.Margin],
        ...
        sf(fig, 340, legend=False).update_layout(
            title=dict(text=f"Net Profit Margin {latest_yr} (Live)", font=dict(size=12,color='#6b80a0')),
    ...
    with c1:
        latest_rev = ann_df[ann_df.Year==ann_df.Year.max()][ann_df.Company.isin(sel_companies)]
        fig = px.treemap(
        ...
        fig.update_layout(**PL, height=330,
            title=dict(text="Revenue Treemap — Color = Net Income (Live)", font=dict(size=12,color='#6b80a0')),
            coloraxis_showscale=False
        )
        st.plotly_chart(fig, use_container_width=True, config={'displayModeBar':False})

    with c2:
        ann_e = ann_df[(ann_df.Year==ann_df.Year.max())&(ann_df.Company.isin(sel_companies))].copy()
        ann_e['RevPerEmp'] = (ann_e.Revenue_B * 1e9 / (ann_e.Employees_K * 1e3) / 1e6).round(2)
        ann_e = ann_e.sort_values('RevPerEmp', ascending=True)
        fig = go.Figure()
        fig.add_trace(go.Bar(
            x=ann_e.RevPerEmp, y=ann_e.Company, orientation='h',
            marker=dict(color=[COLORS[c] for c in ann_e.Company], line=dict(width=0)),
            text=[f"${v:.2f}M" for v in ann_e.RevPerEmp],
            textposition='outside', textfont=dict(size=10, color='#6b80a0'),
            hovertemplate='<b>%{y}</b><br>$%{x:.2f}M per employee<extra></extra>'
        ))
        sf(fig, 330, legend=False).update_layout(
            title=dict(text="Revenue per Employee ($M) — Live Headcount", font=dict(size=12,color='#6b80a0')),
...
# ── PAGE: STOCK PERFORMANCE ───────────────────────────────────────────────────
    ...
    with tab1:
        ...
        sf(fig, 420).update_layout(
            title=dict(text=f"{co1} — Live Price + Bollinger Bands + Moving Averages", font=dict(size=12,color='#6b80a0')),
    ...
    with tab2:
        ...
        sf(fig, 340).update_layout(
            title=dict(text="30-Day Rolling Volatility (Live Daily Returns)", font=dict(size=12,color='#6b80a0')),
        ...
        sf(fig2, 340).update_layout(
            title=dict(text="Risk vs Total Return — Live Stock History", font=dict(size=12,color='#6b80a0')),
    ...
    with tab3:
        ...
        sf(fig, 360, legend=False).update_layout(
            title=dict(text="Annual Stock Return % — Live History", font=dict(size=12,color='#6b80a0')))
        st.plotly_chart(fig, use_container_width=True, config={'displayModeBar':False})
        ...
        sf(fig2, 340).update_layout(
            title=dict(text="Daily Return Distribution — Live Stock History", font=dict(size=12,color='#6b80a0')),
...
# ── PAGE: REVENUE & EARNINGS ──────────────────────────────────────────────────
    ...
    with tab1:
        ...
        sf(fig, 440).update_layout(
            title=dict(text=f"{co2} — Quarterly Revenue + YoY Growth (Live)", font=dict(size=12,color='#6b80a0')))
    ...
    with tab2:
        ...
        for co in sel_companies:
            ...
            if len(sub)==2:
                ...
                cagr_rows.append({'Company':co,'CAGR':round(cagr,1),str(yr_min):r0,str(yr_max):r1})
        ...
        with c1:
            ...
            sf(fig, 340, legend=False).update_layout(
                title=dict(text=f"Revenue CAGR {yr_min}–{yr_max} (Live)", font=dict(size=12,color='#6b80a0')),
                xaxis_title="CAGR %")
            st.plotly_chart(fig, use_container_width=True, config={'displayModeBar':False})
    ...
    with tab3:
        ...
        sf(fig, 360).update_layout(
            title=dict(text="Net Income — Live Earnings Data", font=dict(size=12,color='#6b80a0')),
            yaxis_title="Net Income ($B)")
        st.plotly_chart(fig, use_container_width=True, config={'displayModeBar':False})

        c1, c2 = st.columns(2)
        with c1:
            yr = st.selectbox("Year", sorted(ann_df.Year.unique().tolist(), reverse=True), key='yr_ni')
            ...
            sf(fig2, 320, legend=False).update_layout(
                title=dict(text=f"Revenue vs Net Income {yr} (Live)", font=dict(size=12,color='#6b80a0')),
        ...
        with c2:
            ...
            ni_change = ni_change.copy()
            ni_change['NI_Change'] = ni_change.groupby('Company')['NetIncome_B'].diff()
            ni_latest = ni_change[ni_change.Year==ni_change.Year.max()].sort_values('NI_Change')
            fig3 = go.Figure(go.Bar(
                x=ni_latest.NI_Change, y=ni_latest.Company, orientation='h',
                marker=dict(
                    color=['#00ff9d' if v>=0 else '#ff375f' for v in ni_latest.NI_Change],
                    line=dict(width=0)
                ),
                text=[f"${v:+.1f}B" for v in ni_latest.NI_Change],
                textposition='outside', textfont=dict(size=10,color='#6b80a0'),
                hovertemplate='<b>%{y}</b><br>Change: $%{x:+.1f}B<extra></extra>'
            ))
            sf(fig3, 320, legend=False).update_layout(
                title=dict(text="Net Income YoY Change ($B) — Live", font=dict(size=12,color='#6b80a0')))
...
# ── PAGE: COMPETITIVE ANALYSIS ────────────────────────────────────────────────
    ...
    norm = ann_latest[ann_latest.Company.isin(sel_companies)].set_index('Company')[metrics].copy()
    for col in metrics:
    ...
    ), title=dict(text="Competitive Radar — 5 Dimensions Live (0–10)", font=dict(size=12,color='#6b80a0')))
    st.plotly_chart(fig, use_container_width=True, config={'displayModeBar':False})

    c1, c2 = st.columns(2)
    with c1:
        sec("Rankings Board", f"{latest_yr} LIVE")
        ann_rs = ann_latest[ann_latest.Company.isin(sel_companies)].sort_values('MarketCap_B',ascending=False)
        ...
        <div style="background:var(--bg-card);border:1px solid var(--border);border-radius:14px;padding:1.2rem;">
          <div style="font-family:'JetBrains Mono',monospace;font-size:0.6rem;color:#2d3f5a;letter-spacing:0.1em;margin-bottom:0.8rem;">
            RANKED BY MARKET CAP · LIVE
          </div>{html}</div>""", unsafe_allow_html=True)

    with c2:
        sec("Price/Sales Ratio", "LIVE P/S")
        ann_ps = ann_latest[ann_latest.Company.isin(sel_companies)].copy()
        ann_ps['PS_Ratio'] = ann_ps.MarketCap_B / ann_ps.Revenue_B
        ann_ps = ann_ps.sort_values('PS_Ratio', ascending=True)
        fig = go.Figure(go.Bar(
        ...
        sf(fig, 330, legend=False).update_layout(
            title=dict(text="Price/Sales Ratio — Live Market Cap ÷ Revenue", font=dict(size=12,color='#6b80a0')),
...
# ── PAGE: DEEP ANALYTICS ──────────────────────────────────────────────────────
    ...
    with tab1:
        ...
        sf(fig, 420, legend=False).update_layout(
            title=dict(text="Daily Return Correlation — Live History", font=dict(size=12,color='#6b80a0')))
        st.plotly_chart(fig, use_container_width=True, config={'displayModeBar':False})
    ...
    with tab2:
        ...
        with c1:
            ...
            sf(fig, 300).update_layout(
                title=dict(text=f"{co_qq} — Q-Q Plot vs Normal (Live)", font=dict(size=12,color='#6b80a0')),
        ...
        with c2:
            ...
            sf(fig2, 300).update_layout(
                title=dict(text=f"{co_qq} Daily Returns Distribution (Live)", font=dict(size=12,color='#6b80a0')),
                yaxis_title="Count")
            st.plotly_chart(fig2, use_container_width=True, config={'displayModeBar':False})
    ...
    with tab3:
        ...
        for co in sel_companies:
            ...
            fig.add_trace(go.Scatter(x=sub.Date, y=drawdown, name=co,
                line=dict(color=COLORS[co], width=1.5),
                hovertemplate=f'<b>{co}</b> %{{x|%b %Y}}<br>Drawdown: %{{y:.1f}}%<extra></extra>'))
        sf(fig, 380).update_layout(
            title=dict(text="Maximum Drawdown from Peak — Live History", font=dict(size=12,color='#6b80a0')),
...
# ── PAGE: AI INSIGHT ENGINE ───────────────────────────────────────────────────
    ...
    <div style="background:var(--bg-card);border:1px solid var(--border2);border-radius:14px;
      ...
      <div style="color:var(--txt2);font-size:0.88rem;line-height:1.7;">
        These insights are computed directly from live yfinance earnings & stock data.
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    latest_yr = int(ann_df.Year.max())
    prev_yr   = latest_yr - 2
    ann_latest_idx = ann_df[ann_df.Year==latest_yr].set_index('Company')
    ann_prev_idx   = ann_df[ann_df.Year==ann_df.Year.min()].set_index('Company')

    insights = []
    try:
        nvda_mc_now  = ann_latest_idx.loc['NVIDIA','MarketCap_B']
        nvda_mc_then = ann_prev_idx.loc['NVIDIA','MarketCap_B']
        nvda_growth  = (nvda_mc_now / nvda_mc_then - 1)*100
        nvda_ni      = ann_latest_idx.loc['NVIDIA','NetIncome_B']
        insights.append(('🚀 NVIDIA AI Supercycle',
            f"NVIDIA's market cap grew from ${nvda_mc_then:.0f}B to "
            f"<b style='color:#00ff9d;'>${nvda_mc_now:,.0f}B</b> — a "
            f"<b style='color:#00ff9d;'>{nvda_growth:.0f}% gain</b>. "
            f"Net income hit <b style='color:#00ff9d;'>${nvda_ni:.1f}B</b> on AI chip demand."))
    except: pass
    try:
        meta_ni  = ann_latest_idx.loc['Meta','NetIncome_B']
        insights.append(('💎 Meta\'s Efficiency Era',
            f"Meta rebounded to <b style='color:#00e5ff;'>${meta_ni:.1f}B</b> net income "
            f"driven by headcount cuts and AI-powered ad targeting."))
    except: pass
    try:
        aapl_rev  = ann_latest_idx.loc['Apple','Revenue_B']
        aapl_ni   = ann_latest_idx.loc['Apple','NetIncome_B']
        aapl_mgn  = aapl_ni/aapl_rev*100
        insights.append(('🍎 Apple\'s Revenue Machine',
            f"Apple generated <b style='color:#e8e8e8;'>${aapl_rev:.1f}B</b> revenue "
            f"with a net margin of <b style='color:#e8e8e8;'>{aapl_mgn:.1f}%</b> — "
            f"one of the most profitable companies on earth."))
    except: pass
    try:
        amzn_ni = ann_latest_idx.loc['Amazon','NetIncome_B']
        insights.append(('📦 Amazon\'s Profit Inflection',
            f"Amazon reached <b style='color:#ff9900;'>${amzn_ni:.1f}B</b> net income "
            f"as AWS cloud margins and advertising revenue drive profitability."))
    except: pass
    try:
        tsla_vol = price_df[price_df.Company=='Tesla']['Daily_Return'].std()
        insights.append(('⚡ Tesla: Highest Volatility',
            f"Tesla has daily return std dev of <b style='color:#cc0000;'>{tsla_vol:.2f}%</b> — "
            f"the highest among all 8 companies. Live data confirms continued volatility."))
    except: pass
    ...
    sec("Full Company Scorecard — Live", "LIVE METRICS")
    score = ann_df[ann_df.Year==latest_yr][ann_df.Company.isin(sel_companies)].copy()
    ...
    if not fund_df.empty:
        for _, fr in fund_df.iterrows():
            ...
            if mask.any():
                score.loc[mask,'MarketCap_B'] = fr['marketCap_B']
...
# ── PAGE: LIVE DASHBOARD ──────────────────────────────────────────────────────
    ...
    if _autorefresh_ok:
        st_autorefresh(interval=60000, limit=None, key="live_dashboard_refresh")
    else:
    ...
    for idx, (name, ticker) in enumerate(ticker_map.items()):
        ...
        if price is not None:
            ...
            <div class="kpi" style="margin-bottom:0.8rem;">
              ...
              <div class="kpi-sub">Market closed or unavailable</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with live_col2:
        selected_name    = st.selectbox("Select Company", list(ticker_map.keys()), key="live_company_select")
        interval_choice  = st.selectbox("Interval", ["1m","5m","15m","30m","1h"], index=1, key="live_interval")
        period_choice    = st.selectbox("Period",   ["1d","5d","1mo"],              index=0, key="live_period")
    ...
    with live_col1:
        if df_intraday is not None and not df_intraday.empty:
            ...
            fig_vol = go.Figure(go.Bar(x=df_intraday.index, y=df_intraday['Volume'],
            ...
            <div style="background:var(--bg-card);border:1px solid rgba(255,109,45,0.3);border-radius:14px;
              ...
              <div style="font-family:'Syne',sans-serif;font-size:1rem;font-weight:700;color:#ff6d2d;">Market Closed / No Intraday Data</div>
              <div style="color:#6b80a0;font-size:0.85rem;margin-top:0.5rem;">US markets open Mon–Fri 9:30am–4:00pm ET.<br>Try switching Period to "5d".</div>
            </div>
            """, unsafe_allow_html=True)
    ...
    <div style="font-family:'JetBrains Mono',monospace;font-size:0.58rem;color:#2d3f5a;
      ...
      📡 Live data powered by yfinance · Prices delayed up to 15 minutes during market hours
    </div>
    """, unsafe_allow_html=True)
...
<div style="border-top:1px solid rgba(0,229,255,0.06);margin-top:3rem;padding-top:1.2rem;
  ...
  Data: yfinance Live Feed · Public Earnings Reports · SEC Filings · Historical Stock Records
</div>
""", unsafe_allow_html=True)
